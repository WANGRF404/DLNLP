...
class ChineseDataSet:
    def __init__(self, name):
        ...
        with open("d:/Machine Learning/zipflaw_corpus/cn_stopwords.txt", "r", encoding='utf-8') as f:
    ...
    def read_file(self, filename=""):
        ...
        if filename == "":
            filename = self.name
        target = 'd:/Machine Learning/zipflaw_corpus/' + filename + ".txt"
    ...
    def write_file(self):
        # 将文件内容写入总文件
        target = "d:/Machine Learning/zipflaw_corpus/data.txt"
...
if __name__ == "__main__":
    data_set_list = []
    # 每次运行程序将总内容文件清空
    with open("d:/Machine Learning/zipflaw_corpus/data.txt", "w") as f:
        f.close()
    with open("log.txt", "w") as f:
        f.close()
    # 读取小说名字
    with open("d:/Machine Learning/zipflaw_corpus/inf.txt", "r") as f:
